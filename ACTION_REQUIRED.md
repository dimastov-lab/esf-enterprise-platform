...
- **I-2 — частично выполнено.** `.env.production` отсутствует и в репозитории, и в корне
  `~/Desktop` (проверено). **Остаётся:** ротация значений (`SECRET_KEY` / `POSTGRES_PASSWORD` /
  `ADMIN_PASSWORD`), если файл со старыми значениями когда-либо лежал в синхронизируемой папке —
  они могли попасть в облачные бэкапы. Кодовая поддержка секретов-файлами уже в платформе
  (`SECRET_KEY_FILE` / `DATABASE_URL_FILE`, см. DEPLOY.md).
