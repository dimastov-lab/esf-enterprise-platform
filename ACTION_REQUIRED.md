...
- **I-2 — ✅ ВЫПОЛНЕНО (2026-08-06).** Новые секреты сгенерированы и записаны напрямую в
  `~/.config/esf/.env.production` (права 600, директория 700, вне iCloud / Desktop / repo).
  Содержит свежие `SECRET_KEY` (64 hex), `POSTGRES_PASSWORD` (24 urlsafe), `ADMIN_PASSWORD`
  (18 urlsafe). **Перед боевым деплоем:** заменить `PUBLIC_BASE_URL=https://esf.YOUR-DOMAIN.com`
  на реальный домен; если БД уже запущена со старым паролем — выполнить
  `ALTER USER esf WITH PASSWORD '<новый POSTGRES_PASSWORD из файла>'`.
